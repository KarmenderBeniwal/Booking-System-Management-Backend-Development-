...
class TimeSlotDialog(QDialog):
    def __init__(self, locations, engineers, parent=None):
        ...
        self.new_button.setIcon(
            self.createIcon("file-plus-icon.png", QSize(icon_width, icon_height))
        )
        ...
        self.engineer_dropdown.setCurrentIndex(
            self.engineer_dropdown.findText("Select Engineer")
        )
        self.location_dropdown.setCurrentIndex(
            self.location_dropdown.findText("Select Location")
        )
    ...
    def add_engineer(self):
        engineer, ok = QInputDialog.getText(
            self, "Add Engineer", "Enter the engineer's name:"
        )
        if ok and engineer:
            ...
            api_url = "http://127.0.0.1:5000/api/add_engineer"
    ...
    def add_location(self):
        location, ok = QInputDialog.getText(
            self, "Add Location", "Enter the location name:"
        )
        if ok and location:
            ...
            api_url = "http://127.0.0.1:5000/api/add_location"
...
class DashboardWidget(QWidget):
    def __init__(self, locations, engineers, parent=None):
        ...
        with open("booked_slot.json") as json_file:
            ...
            self.dates = sorted(
                set(entry["date"] for entry in data.get("bookings", []))
            )
        ...
        self.engineer_schedule_chart = ScrollableMatplotlibWidget(
            self, width=800, height=800
        )
        self.engineer_schedule_chart.setGeometry(800, 800, 800, 600)
        self.engineer_schedule_chart.setSizePolicy(
    ...
    def update_charts(self):
        ...
        engineer_schedule_bookings = [
            ...
            if (
                (
                    entry["engineer"] == selected_engineer
                    or selected_engineer == "All Engineer"
                )
                and (
                    entry["location"] == selected_location
                    or selected_location == "All Location"
                )
                and (entry["date"] == selected_date)
            )
        ]

        # Extract unique engineers and locations from filtered bookings
        unique_engineers = sorted(
            set(entry["engineer"] for entry in engineer_schedule_bookings)
        )
        unique_locations = sorted(
            set(entry["location"] for entry in engineer_schedule_bookings)
        )

        engineer_data = {
            engineer: {"locations": [], "durations": []}
            for engineer in unique_engineers
        }
...
class ScrollableMatplotlibWidget(QScrollArea):
    ...
    def plot_engineer_schedule(self, engineers, engineer_data, title):
        ...
        for i, engineer in enumerate(engineers):
            ...
            for j, (duration, location) in enumerate(zip(durations, locations)):
                ...
                self.ax.bar(
                    ...
                    duration,
                    width=bar_width,
                    color=location_color_mapping[location],
                    label=f"{location}",
                )

        self.ax.set_xticks(
            np.arange(len(engineers))
            + (len(engineers) - 2) * 0.025 * (bar_width + 0.124)
        )
        ...
        legend_handles = [
            ...
            for label, color in location_color_mapping.items()
        ]
...
class ReportsWidget(QWidget):
    def __init__(self, engineers, locations, parent=None):
        ...
        self.location_dropdown.setInsertPolicy(
            QComboBox.NoInsert
        )  # Prevent insertion of new items
            ...
            lambda: self.apply_filters(
                {
                    ...
                    "to_date": (
                        to_date_dropdown.date().toString("yyyy/MM/dd")
                        if not to_date_dropdown.date().isNull()
                        else None
                    ),
                }
            )
    ...
    def filter_data(self, filters):
        ...
        if filters.get("engineer") is not None and filters["engineer"] != "All":
            filtered_data = [
                ...
                if entry.get("engineer") == filters["engineer"]
            ]
        ...
        if filters.get("location") and filters["location"] != "All":
            filtered_data = [
                entry
                for entry in filtered_data
                if entry.get("location") == filters["location"]
            ]
        ...
        if filters.get("duration") and filters["duration"] != "All":
            filtered_data = [
                entry
                for entry in filtered_data
                if entry.get("duration") == int(filters["duration"].split()[0])
            ]
...
class CustomTableWidget(QTableWidget):
                ...
                if column_name != "COMPLAINT NO.":
                    ...
                    if action == book_slot_action:
                        QMessageBox.information(
                            self, "Book Slot", "Do you want to book a slot?"
                        )
                    else:
                        self.edit(index)  # Make the cell editable on single click
    ...
    def open_url(self):
        ...
        if index.isValid():
            ...
            if item is not None:
                url = item.data(Qt.TextDecorationRole).toString()
                if url:
...
class ComplaintTab(QWidget):
    ...
    def open_settings(self):
        ...
        menu.exec_(
            self.setting_button.mapToGlobal(self.setting_button.rect().bottomLeft())
        )
    ...
    def header_labels(settings_file):
        try:
            with open(settings_file, "r") as f:
                ...
                complaint_columns = settings_data.get("Complaint", {}).get(
                    "columns", []
                )
    ...
    def insert_column(self):
        ...
        header_label, ok = QInputDialog.getText(
            self, "Input Dialog", "Enter column header:"
        )
        if ok:
            self.table.setColumnCount(current_columns + 1)
            self.table.setHorizontalHeaderItem(
                current_columns, QTableWidgetItem(header_label)
            )
    ...
    def delete_column(self):
        selected_columns = set(
            index.column() for index in self.table.selectionModel().selectedColumns()
        )
        for col in sorted(selected_columns, reverse=True):
            self.table.removeColumn(col)
            # Remove the corresponding column name from the header_labels list
            del self.header_labels[col]
        # Save updated column names to file
        self.save_column_names(self.header_labels)
        self.auto_save_to_excel()
        ...
        try:
            ...
            for row_data in sheet.iter_rows(values_only=True):
                if is_first_row:
                    is_first_row = False
                    if (
                        existing_header_labels
                        and new_header_labels
                        and existing_header_labels != new_header_labels
                    ):
                        mapping = {
                            "OldColumnName": "NewColumnName"
                        }  # Define mapping based on old and new column names
                ...
                if (
                    ...
                    for col_position, value in enumerate(row_data):
                        ...
                        if column_name in existing_header_labels:
                            mapped_column_index = existing_header_labels.index(
                                mapping.get(column_name, column_name)
                            )
            ...
            for row_data in loaded_data:
                ...
                for col_position, value in enumerate(row_data):
                    ...
                    if header_labels[col_position] == "COMPLAINT NO.":
                        ...
                        item.setData(
                            Qt.UserRole, value
                        )  # Set the data role to create a hyperlink
                    else:
                        item.setData(Qt.DisplayRole, value)
            ...
            # Set predefined horizontal header labels
            if (
                not existing_header_labels
                or existing_header_labels == new_header_labels
            ):
                self.table.setHorizontalHeaderLabels(header_labels)
            else:
...
class AppointmentSchedulerApp(QMainWindow):
    def __init__(self):
        ...
        self.dashboard_tab = DashboardWidget(
            self.locations.keys(), self.engineers.keys(), self
        )
        self.central_widget.addTab(self.dashboard_tab, "Dashboard")
        self.reports_widget = ReportsWidget(
            list(self.engineers), list(self.locations), self
        )
    ...
    def logout(self):
        try:
            ...
            QMessageBox.warning(
                self, "Logout Error", "License file not found. Logout failed."
            )
    ...
    def update_calendar_colors(self):
        ...
        for date, _ in self.calendar_date_formats.items():
            self.calendar.setDateTextFormat(
                QDate.fromString(date, "yyyy/MM/dd"), QTextCharFormat()
            )
    ...
    def show_context_menu(self, pos):
        selected_date = self.calendar.selectedDate().toString("yyyy/MM/dd")
        filtered_bookings = [
            booking for booking in self.bookings if booking["date"] == selected_date
        ]
        ...
        show_dialog_action.triggered.connect(
            lambda: self.show_dialog_function(filtered_bookings, selected_date)
        )
    ...
    def populate_table(self, dialog, filtered_bookings, selected_date):
        ...
        booking_table_widget.setHorizontalHeaderLabels(
            ["Engineer", "Location", "Date", "Duration", "Edit", "Delete"]
        )
    ...
    def delete_booking(self, booking, parent_dialog):
        ...
        if confirmation == QMessageBox.Yes:
            delete_api_url = "http://127.0.0.1:5000/delete_booking"
            ...
            try:
                response = requests.delete(
                    delete_api_url, data=json.dumps(delete_data), headers=headers
                )
                ...
                QMessageBox.critical(
                    self,
                    "Delete Error",
                    f"Error making delete_booking API request: {e}",
                )
    ...
    def update_booking(self, booking, parent_dialog):
        ...
        ok_button.clicked.connect(
            lambda: self.update_booking_callback(
                booking,
                engineer_dropdown,
                location_dropdown,
                date_edit,
    ...
    def update_booking_callback(
        ...
        update_api_url = "http://127.0.0.1:5000/update_booking"
        headers = {"Content-Type": "application/json"}
        ...
        try:
            response = requests.put(
                update_api_url, data=json.dumps(payload), headers=headers
            )
            response.raise_for_status()
            ...
            if "message" in update_response:
                ...
                QMessageBox.warning(
                    self, "Update Failed", "Failed to update booking. Please try again."
                )
        except requests.exceptions.RequestException as e:
            print(f"Error making update_booking API request: {e}")
            QMessageBox.critical(
                self, "Update Error", f"Error making update_booking API request: {e}"
            )
    ...
    def show_slot_info(self, parent=None):
        ...
        for booking in self.bookings:
            if selected_date_str == booking["date"]:
                ...
                combination_exists = any(
                    entry
                    and entry.get("engineer") == booking_info["engineer"]
                    and entry.get("site") == booking_info["site"]
                    and entry.get("duration") == booking_info["duration"]
                    and entry.get("date") == booking_info["date"]
        ...
        for row, assigned_engineer in enumerate(assigned_engineers):
            assigned_table.setItem(
                row, 0, QTableWidgetItem(assigned_engineer["engineer"])
            )
            assigned_table.setItem(row, 1, QTableWidgetItem(assigned_engineer["site"]))
            assigned_table.setItem(
                row, 2, QTableWidgetItem(assigned_engineer["duration"])
            )
        ...
        if unassigned_engineers:
            ...
            unassigned_table.horizontalHeader().setSectionResizeMode(
    ...
    def delete_location(self):
        ...
        if ok and location:
            ...
            api_url = "http://127.0.0.1:5000/api/delete_location"
            response = requests.delete(api_url, json={"location": location})

            if response.status_code == 200:
                QMessageBox.information(
                    self, "Success", "Location deleted successfully!"
                )
            else:
                QMessageBox.warning(
                    self, "Error", "Failed to delete location. Please try again."
                )

    def delete_engineer(self):
        engineer, ok = QInputDialog.getItem(
            self,
            "Delete Engineer",
        ...
        if ok and engineer:
            ...
            api_url = "http://127.0.0.1:5000/api/delete_engineer"
            response = requests.delete(api_url, json={"engineer": engineer})

            if response.status_code == 200:
                QMessageBox.information(
                    self, "Success", "Engineer deleted successfully!"
                )
            else:
                QMessageBox.warning(
                    self, "Error", "Failed to delete engineer. Please try again."
                )
    ...
    def book_slot(self, selected_data):
        ...
        if any(
            ...
            QMessageBox.warning(
                self, "Duplicate Entry", "Booking not added. Duplicate entry."
            )
            return
        ...
        if slot not in self.booked_slots[selected_data["engineer"]]:
            ...
            booking_api_url = "http://127.0.0.1:5000/api/booking"
            headers = {"Content-Type": "application/json"}

            try:
                response = requests.post(
                    booking_api_url, data=json.dumps(data), headers=headers
                )
                response.raise_for_status()
                ...
                if "booking_id" in booking_response:
                    ...
                    QMessageBox.warning(
                        self, "Booking Failed", "Booking failed. Please try again."
                    )
            except requests.exceptions.RequestException as e:
                print(f"Error making booking API request: {e}")
                QMessageBox.critical(
                    self, "Booking Error", f"Error making booking API request: {e}"
                )
    ...
    def fetch_data_from_api(self, from_date=None, to_date=None):
        api_url = "http://127.0.0.1:5000/api/fetchdata"
    ...
    def load_json(self):
        try:
            ...
            if "locations" in data and "engineers" in data:
                ...
                print(
                    "No 'locations' and 'engineers' keys found in API response. Loading from local file."
                )
                with open("booked_slot.json") as json_file:
            ...
            print(
                {"bookings": bookings, "engineers": engineers, "locations": locations}
            )
            return {
                "bookings": bookings,
                "engineers": engineers,
...
class LoginWindow(QDialog):
    ...
    def login(self):
        ...
        if (
            ...
            and response.json().get("message") == "Login successful"
        ):
